...
{
  // 기존 로직 유지 (int8_t 및 size_t 경고 주의)
  ...
  for (int8_t size = 0; size < (int8_t)manipulator->getDOF(); size++) // 캐스팅으로 경고 방지
  {
    ...
    index++;
    // ROS2 Humble 등에서 vector 크기 체크 추가 권장
    if(!manipulator->getComponentChildName(my_name).empty())
...
bool SolverUsingCRAndJacobian::inverseSolverUsingJacobian(Manipulator *manipulator, Name tool_name, Pose target_pose, std::vector<JointValue> *goal_joint_value)
{
  ...
  for (int8_t count = 0; count < iteration; count++)
  {
    ...
    //get delta angle
    Eigen::ColPivHouseholderQR<Eigen::MatrixXd> dec(jacobian);
